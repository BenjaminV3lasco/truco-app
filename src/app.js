...
const state = { screen:'home', step:1, names:[''], mode:'', format:'', groups:[], music:false, score:{ us:0, them:0, target:30, winner:null, winnerOpen:false } };
...
root.addEventListener('click',e=>{
  ...
  if(score){const side=score.dataset.score;if(!state.score.winner&&state.score[side]<state.score.target){state.score[side]++;if(state.score[side]===state.score.target){state.score.winner=side;state.score.winnerOpen=true;}}render();return;}
